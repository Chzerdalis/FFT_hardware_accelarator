...
module multiplier_sythesis_2 #(
    ...
    // =========================================================================
    reg [PAD_WIDTH-1:0] cpa_a_pipe      [0:NCHUNKS-1];
    reg [PAD_WIDTH-1:0] cpa_b_pipe      [0:NCHUNKS-1];
    reg [PAD_WIDTH-1:0] cpa_result_pipe [0:NCHUNKS-1];
    reg                 cpa_carry_pipe  [0:NCHUNKS-1];
    reg                 cpa_valid_pipe  [0:NCHUNKS-1];

    wire [PAD_WIDTH-1:0] csa_sum_pad;
    wire [PAD_WIDTH-1:0] csa_carry_pad;

    assign csa_sum_pad   = {{(PAD_WIDTH-P_WIDTH){1'b0}}, csa_sum_r};
    assign csa_carry_pad = {{(PAD_WIDTH-P_WIDTH){1'b0}}, csa_carry_r};
    ...
    always @(posedge clock) begin
        if (reset) begin
            ...
            // =============================================================
            valid_pipe[0] <= start;
            a_pipe[0]     <= {{B_WIDTH{a[WIDTH-1]}}, a};
            b_pipe[0]     <= b;

            if (b[0])
                comb_pp = {{B_WIDTH{a[WIDTH-1]}}, a};
            else
                comb_pp = {P_WIDTH{1'b0}};

...
module SdfUnit2_fast #(
    ...
    wire [KEY_WIDTH - 1:0] wr, wi;

    (* ram_style = "distributed" *) reg [KEY_WIDTH - 1:0] w_real [0: twiddle_array_size -1];
    (* ram_style = "distributed" *) reg [KEY_WIDTH - 1:0] w_imag [0: twiddle_array_size -1];

    
    initial begin
        $readmemh(twiddle_file_real, w_real);
        $readmemh(twiddle_file_imag, w_imag);
    end

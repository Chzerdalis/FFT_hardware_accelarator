...
with open(f'f_input_rev_{N}_32bit.vh', 'w') as f:
    for i, xi in enumerate(x_32bit):
        xir = int(xi)
        f.write(f'gen_input_real[{bitrev(i, int(np.log2(N)))}] = {sign(xir):}32\'sd{abs(xir)};\n')

# with open(f'f_input_rev_{N}_32bit_.vh', 'w') as f:
#     for i in range(N):
#         y = bitrev(i, int(np.log2(N)))
#         xir = int(x_32bit[y])
#         f.write(f'{sign(xir):}{abs(xir)}\n')

with open(f'f_input_rev_{N}_24bit.vh', 'w') as f:
    for i, xi in enumerate(x_24bit):
        xir = int(xi)
        f.write(f'gen_input_real[{bitrev(i, int(np.log2(N)))}] = {sign(xir):}24\'sd{abs(xir)};\n')

with open(f'f_input_rev_{N}_18bit.vh', 'w') as f:
    for i, xi in enumerate(x_18bit):
        xir = int(xi)
        f.write(f'gen_input_real[{bitrev(i, int(np.log2(N)))}] = {sign(xir):}18\'sd{abs(xir)};\n')

with open(f'f_input_rev_{N}_16bit.vh', 'w') as f:
    for i, xi in enumerate(x_16bit):
        xir = int(xi)
        f.write(f'gen_input_real[{bitrev(i, int(np.log2(N)))}] = {sign(xir):}16\'sd{abs(xir)};\n')
################################

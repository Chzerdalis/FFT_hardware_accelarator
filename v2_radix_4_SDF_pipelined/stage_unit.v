...
module SdfUnit4_fast #(
    ...
    //Having it in ram distributed to allow synthesis in FPGAs without block RAMs which is faster for
    //Our memory needs which are small
    ...
    //Twiddle factor ROM instances, if sythesys can synthesize the
    //.vh into ROMs this is not needed
    ...
    //Registers for counters and twiddle indexes
    always @(posedge clock) begin
        if (reset) begin
    ...
    //Input registers and other needed registers
    always @(posedge clock) begin
        if (reset) begin
